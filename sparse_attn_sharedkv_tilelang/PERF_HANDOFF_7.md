...
#### 0.6.4d cube-scalar 探针 = 负结果 → resonance 墙坐实,fork unitFlag 不划算,锁 46.5（2026-06-17）

**用户选「先便宜 kernel 探针验证耦合再决定动 fork」。** Plan-agent 深挖结论:① `max_seq` 是**编译期常量**(`max_seq: int` 参数,JIT 特化)→ decode 的 `//`/`% max_seq` 是常量除法、降成 mul-shift、**本来就便宜**,不是 scalar 成本;② 49% cube scalar 的大头是**软件 flag 发射**(~24 set/wait/cross 每步,只有 fork unitFlag 能去)+ GM 标量载入 + selects;③ decode 跨迭代 carry 在 cube **不可行**(AIC 无 UB scratch、无持久标量)。唯一可行的便宜 cube-scalar 刀 = batch==1 时把循环不变的 `actual_q_len[0]`/`actual_kv_len[0]`/`q_prefix[0]` 外提(b_safe 恒 0),省每步 5 个 GM 标量载入。

**探针容器结果**:decode/dtype0✅;**swa 46.5→45.5(−1.0)**,cfa 43.6→44.1(+0.5,但 cfa 走没动的旧 prim_func ⇒ **噪声地板 ~±0.5**),scfa 平。**⇒ 砍 cube scalar(GM-load)不仅没帮上、还略伤**。**和 C2(削 gather scalar = flat)同剧本 = 第二次坐实「kernel-local cube 刀打不动 kernel」**。

**根因 = cross-flag resonance(skill 烧过 4 次的陷阱)**:cube 95% util 没错,但**临界路径是 cube↔vector 的 cross-flag 接力时序,不是 cube 原始工作量**;砍 cube 工作 → 改 cube 产 SCORE/PV_READY 的时刻 → 与 vector 错拍 → 反慢。**这直接否掉 fork unitFlag**:它也是 local cube 刀(且改 cube 内部 sync 时序更猛),大概率撞同一面墙,**不值得动 fork 去赌**。

**已回退探针 → C4 态 swa 46.5(stripped kernel.py,无注释)。结论:kernel-local 优化(cube/vector 两侧的便宜刀 + matmul-tiling + 单点 scalar 刀)经验上全部用尽,~46-50% 是真实平台天花板**(与 _6 §3.5/memory 一致)。**破它只剩结构性大改**(cube↔vector work 重分配 / 改接力流水深度,~55-60 乐观、非 80;是项目不是单刀)**或收手锁 46.5**。北极星 80 在 kernel 层基本够不着,除非结构重写 + 可能的编译器层(都高风险)。**当前 push:probe 回退 + 注释清除(2779→1748)**。

**TileLang 坑(本次新踩,记 [[feedback-kernel-no-comments]])**:prim_func 体里 `if <python-const>:` 被当 TIR If(条件假整块跳过、块内赋值不出块);parser **贪婪 eval 三元两分支** → `X if c else Y` 的 X 必须已定义。修法=无条件定义 + 三元在用点门控。
