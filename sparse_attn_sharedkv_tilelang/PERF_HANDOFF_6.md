...
- **`SoftmaxFlashV2` 融合 → 真 headroom，但是编译器 builtin**：我们 V1 softmax 是 ~8 个独立 VEC pass（`kernel.py` 1294-1351:reduce_max/max/exp(alpha)/broadcast/sub/exp/reduce_sum/rescale），Ascend C 一个 `SoftmaxFlashV2`（`swa_block_vector.h:349-355`，config `SOFTMAX_OUTPUT_WITHOUT_BRC`）全融合。攻当前长板（vector），收益 ~几个点（被 cube 变长板封顶）但忠实复刻 + 攒 tag。
  ...
  **③ 容器验 ✅ 完成（green + 已 tag，但 perf 中性）**：build 第一次失败=`SoftMaxTiling` 未声明 —— 根因：这套 CANN **没有** op-build 的 `include/kernel_operator.h`+`include/lib/`,softmax adv_api 在 **ASC stdlib** `aarch64-linux/asc/include/adv_api/activation/`(`-xasc` 隐式路径,basic op 自动、adv_api opt-in)。fork `a3b4f40` 修:`common.h` 改 include `adv_api/activation/softmax_tiling.h`+`softmaxflashv2.h`(`__has_include` 守 + fallback `kernel_operator.h`),并去掉 softmax 符号的 `AscendC::` 限定(复刻 ops-transformer 写法,靠 `using namespace AscendC`)。`SoftMaxFlashV2TilingFunc` 在此版是 `__aicore__`(device 可调)。**common.h-only 改不用重装 .so,`git pull` 即生效**。**结果**:fast(swa/cfa/scfa prefill)+ `decode and dtype0` 全绿,数值 == manual chain(含 sink seed)。**已打 `cfeat-softmax-flashv2`(a3b4f40,green user=78838ea)**。**但 perf 没动针**:swa 42.8→**42.4** / cfa 49.3→**49.9**(噪声内)、scfa 16.5→15.0(SCFA 逻辑没动=噪声)。印证 vector 只比 cube 长 ~4% 且 cube_direct softmax 早已 debarrier/向量化,8 op 折 1 没气可挤。**决定:保留**(忠实复刻 Ascend C 计算逻辑 + NPU 验过的编译器特性 + kernel 更简洁,无 perf 损失;要回退也可但没理由)。
  **⚠️ 战略结论:kernel 侧 vector 便宜刀至此用尽(A2 同步、brcb/row_muls rescale、SoftmaxFlashV2 全是中性或微调)。真要往 80% 走只剩 §3.5① 跨-slot 软件流水(C,见下),那是结构性大改、all-or-nothing、需用户拍板投容器带宽。** 另:NPU 报 `brcb is deprecated` warning —— Brcb API 未来 CANN 可能移除,届时 brcb/row_muls lever 要迁到替代 API(非紧急,记一笔)。
