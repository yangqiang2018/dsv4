                                    ...
                                    if t >= 2:
                                        ...
                                        for mp in range(N_MERGE_PASS):
                                            ...
                                            if cube_direct:
                                                ...
                                                # per-head form stays.
                                                #
                                                # Vectorizing the MUL instead (broadcast the
                                                # alpha slice -> [MERGE_HEADS, D] + one wide
                                                # mul, fork 1a70fed) was tried too and
                                                # REVERTED: AscendC Broadcast does not fill a
                                                # 512-wide dst -- it leaves ~the last 64-col
                                                # block per row stale, so ~65/512 = 12.7% of
                                                # outputs got a wrong (previous) alpha (prefill
                                                # swa/cfa ~87%, max rel 2.0; the 128-wide V1
                                                # softmax broadcast is under the threshold and
                                                # is fine). The broadcast lowering is correct
                                                # (dump-verified); the AscendC op is the limit.
