        ...
        def sparse_attn_sharedkv_swa(
            ...
            with T.Kernel(core_num, is_npu=True) as (cid, vid):
                ...
                with T.Scope("C"):
                    ...
                    #   BACK_A (fix->m, eid 0) / BACK_B (fix->m, eid 1): L0C is a
                    #     2-slot ping-pong (slot A @0, slot B @64KB). MM1's lo
                    #     score gemm writes acc_s_a (A), hi writes acc_s_b (B), so
                    #     the hi gemm (MAD) overlaps the lo copy (FIX). acc_o
                    #     (128KB) still spans BOTH slots, so MM2 touches A and B.
                    #     Per-slot WAR flag: set after a slot's last FIX read,
                    #     waited before its next MAD write -- alternating
                    #     block-by-block (MM1 writes A@lo / B@hi; MM2 writes A+B;
                    #     reads symmetric) serializes each slot's chain across the
                    #     MM1/MM2 seam. Pre-set arms each first wait; 2 drains
                    #     after the loop. (Same proof as the old single BACK_LC,
                    #     replicated per slot.)
                    ...
                    for g in T.serial(total_work + 2):
                        ...
                        # (MM2 at step g-1, valid1) -> set-count == wait-count. ----
                        ...
                        # clobber buffers step g-1's MM2 still reads. Draining
                        # the MTE2 pipe here (before ANY load) frees kv[g%2]
                        # (MM2(g-2) read), q_l1[g%2] (MM1(g-2) read, transitive
                        # via in-order MAD) and p_lo (MM2(g-2) read, transitive
                        # via in-order MTE2 after this wait). ----
                        if valid2:
                        ...
                        if valid0:
                            ...
                            # Replicate Ascend C's CopyGmToL1 bulk nd2nz DataCopy
                            # (swa_block_cube.h:244): copy each 64-row half in ONE
                            # runtime-extent T.copy per paged-block segment, NOT
                            # GATHER_ROWS=16 chunks. The per-chunk g0//ori_block_size
                            # + g0%ori_block_size were the dominant cube SCALAR cost
                            # (msprof post-C1: cube scalar 51%, the long-pole pipe,
                            # 8+8 runtime div/mod per step). A 64-row half spans <=2
                            # paged blocks when ori_block_size >= BI_half, so this is
                            # 1-2 copies + 1 div + 1 mod per half (2+2 per step). The
                            # chunk offset is 0 (SWA NI_ori == 1), so the lo half
                            # starts at ori_left0, the hi half at ori_left0 + BI_half.
                            ...
                            T.set_flag(
                                "fix", "m", 0
                            )  # BACK_A: slot A free (lo copy done)
                            # hi copy (FIX).
                            T.wait_flag("m", "fix", 2)
                            T.copy(
                                acc_s_b,
                            ...
                            T.set_flag(
                                "fix", "m", 1
                            )  # BACK_B: slot B free (hi copy done)
                            # SCORE_READY ordered by the FIX pipe (both copies).
                            T.set_cross_flag("FIX", _FLAG_SCORE_READY)
                    ...
                    # pre-sets (and consume them unchanged on a zero-valid core).
                    T.wait_flag("fix", "m", 0)
                    T.wait_flag("fix", "m", 1)
